...
/**
 * Tudo o que o mascote PODE dizer a esta usuária hoje.
 *
 * Quem decide o que aparece, e quando, é o cliente (`escolherMensagem`, com o
 * limite diário e o intervalo). Aqui só se reúne o material — e três regras
...
export async function montarMensagensMascote(
  ...
  try {
    ...
    const vivas = acoes
      .filter((a) => situacaoDaAcao(a) !== "encerrada")
      .slice(0, 3);
    ...
    for (const a of vivas) {
      ...
      mensagens.push({
        ...
        emocao: situacao === "hoje" ? "atenta" : "feliz",
      });
    }
    ...
    mensagens.push(...dicasParaHoje(diaDoAno(hoje)));
